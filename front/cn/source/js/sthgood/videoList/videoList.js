...
class ListBox extends React.Component{
    ...
    componentDidMount(){
        fetch(`${apiUrl}shoppingguide/video-cat-list`)
            .then(res=> {
                if (res.status === 200) {
                    return res.json();
                } else {
        ...
        return(
            <div className="sort-nav">
                <ul className="clearfix">
                    <li className={this.activeClass(0)} onClick={this.handleChangeCat.bind(this,0,0)}><a >全部视频</a></li>
                    {
        ...
        if(this.state.videosData.listResult){
            listCont = this.state.videosData.listResult.map((v,i) =>
                <li key={i} className="item">
                    <h3 className="h3"><a href={v.href}>{v.productTitle}</a></h3>
                        ...
                        <div className="subSort-nav clearfix">
                            <div className="sort-btn fl">
                                <ul className="clearfix">
                                    ...
                                    <li className="active" >最新上传</li>
                                    {/*<li ref="sortByplays" onClick={this.sortByplays}>播放数量</li>*/}
                                </ul>
                            </div>
                            <div className="describe fr">
                                <span className="total-num" >共 <span className="num" ref="totalNum">{this.state.videosData.totalRow || 0}</span> 个视频</span>
